#' @title  Columns percentages of a frequency table from variables and clusters
#' @description  This function returns a columns percentages frequency table from variables and clusters with formatting
#'
...
#' cl.profil(var,clust)
#'
cl.profil <- function(active_variables, clusters,nom=NULL, show_graph=TRUE, digits=2){
  ...
  if(is.factor(active_variables) || is.character(active_variables)){
    ...
    for(i in 1:ncol(active_variables)){
      if(is.factor(active_variables[,i]) || is.character(active_variables[,i])){
        ls[[cpt]] = cl.profil(active_variables[,i], clusters,colnames(active_variables)[i])
        cpt = cpt +1
      }
    }
    return(ls)
