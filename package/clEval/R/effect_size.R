...
effect_size <- function(active_variables, clusters, digits=3) {
  if(all(sapply(active_variables, is.numeric))==FALSE) { #Check if input variables are numeric
    ...
    active_variables_clusters <- active_variables %>% dplyr::mutate(clusters = factor(clusters))
    ...
    active_variables_sd <- active_variables_clusters %>%
      group_by(clusters) %>%
      dplyr::summarise_if(.predicate = function(x) is.numeric(x), .funs = list(sd)) %>%
      select_if(function(x) is.numeric(x))

    #Calculate length across all clusters
    active_variables_n <- active_variables_clusters %>%
      group_by(clusters) %>%
      dplyr::summarise_if(.predicate = function(x) is.numeric(x), .funs = list(length)) %>%
      select_if(function(x) is.numeric(x))
    ...
    for(j in 1:k) {
      for(i in 1:c) {
        sel <- active_variables_clusters[which(active_variables_clusters$clusters!=j),] %>% select_if(function(x) is.numeric(x))
        sd1 <- active_variables_sd[j,i] %>% as.numeric()
        sd2 <- as.numeric(sd(sel[,i]))
    ...
    n <- active_variables_clusters %>%
      dplyr::summarise_if(.predicate = function(x) is.numeric(x), .funs = list(length)) %>%
      select_if(function(x) is.numeric(x))
    m <- cluster_mean[-j,] %>% dplyr::summarise_all(mean)
    ...
    m2full <- active_variables_clusters %>%
      dplyr::filter(clusters != j) %>%
      dplyr::summarise_if(.predicate = function(x) is.numeric(x), .funs = list(mean)) %>%
      select_if(function(x) is.numeric(x))
    ...
    print(list("Hedge's g" = es_g %>% as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))))
    #Printing formatted table in Viewer tab (html format)
    print(es_g %>%  as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))%>%
            dplyr::mutate("clusters (Hedge's g)"= 1:k) %>% select("clusters (Hedge's g)", everything()) %>%
    ...
    if(variable != "") {
      ...
      var_clusters <-active_variables_clusters %>% gather(key, value, -clusters) %>% dplyr::filter(key==variable)
      ...
      for(i in 1:k) {
        ...
        assign(paste("df", i, sep = ""), data.frame(x, y)%>%dplyr::mutate(clusters=factor(i))) }
      ...
      assign(paste("df",k+1, sep = ""), data.frame(x= density(var_clusters$value)$x, y= density(var_clusters$value)$y) %>%
               dplyr::mutate(clusters=factor("Variable (unclustersed)")))
      ...
      graph_density_clusters<- df_total %>% dplyr::filter(clusters!="Variable (unclustersed)") %>% ggplot(aes(x=x, y=y, color=clusters))+
      ...
      graph_density_total<-df_total %>% dplyr::filter(clusters=="Variable (unclustersed)") %>% ggplot(aes(x=x, y=y, color=clusters))+
        geom_area(aes(x=x, y=y, fill=clusters),alpha=0.2)+
        geom_line(size=0.75)+
        scale_fill_brewer(palette="Dark2")+
        scale_color_brewer(palette="Dark2")+
      ...
      if(inspect_cluster != "") {
        ...
        df_clust <- data.frame(x= density(var_clusters[which(var_clusters$clusters==inspect_cluster),]$value)$x, y=density(var_clusters[which(var_clusters$clusters==inspect_cluster),]$value)$y ) %>% dplyr::mutate(clusters = inspect_cluster)
        df_clust_other <- data.frame(x= density(var_clusters[which(var_clusters$clusters!=inspect_cluster),]$value)$x, y= density(var_clusters[which(var_clusters$clusters!=inspect_cluster),]$value)$y ) %>% dplyr::mutate(clusters="others")
        ...
        print(ggarrange(graph_density_total, graph_density_clusters, graph_density_cluster_vs_other,
                        ...
                        ncol = 2, nrow=2)) } else {
                          ...
                          for( i in factor(1:k)) {
                            ...
                            #Creating dataframe with density curve coordinates for selected cluster and others clusters if something is selected in the prompt
                            df_clust <- data.frame(x= density(var_clusters[which(var_clusters$clusters==i),]$value)$x, y=density(var_clusters[which(var_clusters$clusters==i),]$value)$y ) %>% dplyr::mutate(clusters = i)
                            df_clust_other <- data.frame(x= density(var_clusters[which(var_clusters$clusters!=i),]$value)$x, y= density(var_clusters[which(var_clusters$clusters!=i),]$value)$y ) %>% dplyr::mutate(clusters="others")
      ...
      #Printing formatted table in Viewer tab (html format)
      print(es_d %>%  as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))%>%
              dplyr::mutate("clusters (Cohen's d)"= 1:k) %>% select("clusters (Cohen's d)", everything()) %>%
              formattable(align = c("l",rep("r", c )),
                          list(`Indicator Name` = formatter("span", style = ~ style(color = "grey",font.weight = "bold")),
                               area(col = 1:c+1) ~ color_tile("#DeF7E9", "#71CA97"))))
      print(u3 %>%  as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))%>%
              dplyr::mutate("clusters (U3)"= 1:k) %>% select("clusters (U3)", everything()) %>%
              formattable(align = c("l",rep("r", c )),
                          list(`Indicator Name` = formatter("span", style = ~ style(color = "grey",font.weight = "bold")),
                               area(col = 1:c+1) ~ color_tile("#DeF7E9", "#71CA97"))))
      print(u2 %>%  as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))%>%
              dplyr::mutate("clusters (U2)"= 1:k) %>% select("clusters (U2)", everything()) %>%
              formattable(align = c("l",rep("r", c )),
                          list(`Indicator Name` = formatter("span", style = ~ style(color = "grey",font.weight = "bold")),
                               area(col = 1:c+1) ~ color_tile("#DeF7E9", "#71CA97"))))
      print(u1 %>%  as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))%>%
              dplyr::mutate("clusters (U1)"= 1:k) %>% select("clusters (U1)", everything()) %>%
              formattable(align = c("l",rep("r", c )),
                          list(`Indicator Name` = formatter("span", style = ~ style(color = "grey",font.weight = "bold")),
                               area(col = 1:c+1) ~ color_tile("#DeF7E9", "#71CA97"))))
      print(besd %>%  as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))%>%
              dplyr::mutate("clusters (BESD)"= 1:k) %>% select("clusters (BESD)", everything()) %>%
              formattable(align = c("l",rep("r", c )),
                          list(`Indicator Name` = formatter("span", style = ~ style(color = "grey",font.weight = "bold")),
                               area(col = 1:c+1) ~ color_tile("#DeF7E9", "#71CA97"))))
      print(cles %>%  as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))%>%
              dplyr::mutate("clusters (CLES)"= 1:k) %>% select("clusters (CLES)", everything()) %>%
              formattable(align = c("l",rep("r", c )),
                          list(`Indicator Name` = formatter("span", style = ~ style(color = "grey",font.weight = "bold")),
                               area(col = 1:c+1) ~ color_tile("#DeF7E9", "#71CA97"))))
      ...
      #Returning tables
      results<-list()
      results$d.cohen<-es_d %>% as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))
      results$g.hedges<-es_g %>% as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))
      results$u3<-u3 %>% as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))
      results$u2<-u2 %>% as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))
      results$u1<-u1 %>% as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))
      results$besd<-besd %>% as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))
      results$cles<-cles %>% as.data.frame() %>% dplyr::mutate_if(is.numeric, ~round(., digits))
